...
def get_uncompiled_logs() -> list[Path]:
    ...
    for log_file in sorted(DAILY_DIR.glob("*.md")):
        ...
        has_content = any(
            tag in content
            for tag in ("[DECISION]", "[PATTERN]", "[MISTAKE]", "[CONCEPT]", "[PROJECT]")
        )
...
4. Update {wiki}/index.md — append new article links under a "### Compiled {today}" heading
5. Update {memory} — add 1-line pointers to new articles (keep file under 50 lines total)
6. Mark each compiled log as compiled by changing "compiled: false" to "compiled: true"
...
def _update_memory(written: list[Path]) -> None:
    ...
    lines = [l for l in content.splitlines() if l and not l.startswith("#") and not l.startswith("<!--")]
    if len(lines) > 45:
...
def _compile_with_api(logs: list[Path], dry_run: bool) -> int:
    ...
    try:
        ...
        print("ERROR: neither claude_agent_sdk nor anthropic package is installed", file=sys.stderr)
        return 0
...
def main() -> None:
    ...
    if not success:
        ...
        if raw:
            # Reuse the same JSON parsing + writing logic as Tier 2
            try:
                if raw.startswith("```"):
                    raw = raw.split("```", 2)[1]
                    if raw.startswith("json"):
