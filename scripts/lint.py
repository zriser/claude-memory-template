...
def check_orphaned_notes(articles: dict[str, str]) -> list[str]:
    ...
    for path in articles:
        if Path(path).name == "index.md" or path.startswith("daily/"):
            continue
        stem = Path(path).stem
        if stem not in referenced and stem.lower() not in referenced and path not in referenced:
        ...
        if date_str:
            try:
                ...
                if note_date < cutoff:
                    ...
                    issues.append(f"STALE ({age}d): `{path}` — last updated {date_str.group(1)}")
            except ValueError:
                pass
        else:
...
def _check_contradictions_with_api(articles: dict[str, str]) -> str:
    ...
    relevant = {
        path: content for path, content in articles.items()
    ...
    try:
        ...
        response = client.messages.create(
            ...
            max_tokens=2048,
            messages=[{"role": "user", "content": FALLBACK_CONTRADICTION_PROMPT.format(content=combined)}],
        )
...
def check_contradictions(articles: dict[str, str]) -> list[str]:
    ...
    if AGENT_SDK_AVAILABLE:
        ...
        try:
            ...
            print(f"Tier 1 failed ({type(e).__name__}), trying Tier 2...", file=sys.stderr)
...
def print_report(
    ...
    total = len(broken) + len(orphans) + len(stale) + len(missing_fm) + len(contradictions)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Lint the Claude Memory knowledge base")
    ...
    parser.add_argument("--stale-only", action="store_true", help="Only report stale notes")
